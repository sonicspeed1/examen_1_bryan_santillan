...
public class usuario {
	 ...
	 public boolean cambio_contraseña(int user,String new_pass) {
		 boolean res=false;
		 String sql="Update tb_usuario set contraseña ='"+new_pass+
				 "' where id_us="+user;
		 try {
			 con.Ejecutar(sql);
			 res=true;
	   ...
	   public String info_donacion(String nombre) {
		    ...
		    try {
		        tabla += "<table style='border-collapse: collapse; width: 100%;'>"
		               ...
		               + "<th style='border: 1px solid black; padding: 8px;'>DONACION(VECES)</th>"
		               + "</tr>";
		        while (rs.next()) {
		            tabla += "<tr>"
	   ...
	   public boolean eliminar(int id_don) {
		   boolean est=false;
		   String sql="DELETE FROM TB_DON WHERE id_don="+id_don;
		   try {
			   est=true;
		   con.Ejecutar(sql);}
		   catch(Exception e){
	 ...
	 public String Lista() {
		 String tabla="";
		 String sql="select id_don,nombre,beneficiario,origen,destino, "
		 		+ "	recurso,estado from tb_don,tb_usuario where id_us=donador "
		 		+ "order by id_don asc;";
		 ResultSet rs=con.Consulta(sql);
		 try {
			 tabla += "<form action='Lista_donadores' method='post'>"+
					 "<table style='border-collapse: collapse; width: 100%; margin-top:10px;'>"
		               + "<tr style='background-color: green; color: white;'>"
		               + "<th style='border: 1px solid black; padding: 8px;'>DONADOR</th>"
			...
			while(rs.next()) {
				tabla += "<tr>"
						  ...
						  + "<td style='border: 1px solid black; padding: 8px;'> <a href= modificar.jsp?cod=" + rs.getInt(1) + "><pre style=\"text-align: center\">Modificar</pre></a></td>" 
						+ "<td style='border: 1px solid black; padding: 8px;'> <a href= eliminar.jsp?cod=" + rs.getInt(1) + " \"><pre style=\"text-align: center\">Eliminar</pre></a></td>"
		                   + "</tr>";
			 }
			   tabla += "</table></form>";
		} catch (SQLException e) {
	...
	public boolean actualizar(int id_don,String ben,String or,String de,String re,String est) {
		boolean rec=false;
		String sql = "update tb_don set beneficiario ='" + ben + 
	             "', origen ='" + or + 
	             "', destino ='" + de + 
	             "', recurso ='" + re + 
	             "', estado ='" + est + 
